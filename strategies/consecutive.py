...
def run(stock_symbol, initial_capital, consecutive_days=3):
    print("\n=== [Step 1] 下載資料 ===")
    ...
    # === [Step 2] 產生交易訊號 (Consecutive Up/Down 策略) ===
    data['Signal'] = 0
    up = data['Close'] > data['Close'].shift(1)
    down = data['Close'] < data['Close'].shift(1)

    # 連續上漲 N 天 → 買入
    data.loc[up.rolling(consecutive_days).sum() == consecutive_days, 'Signal'] = 1
    # 連續下跌 N 天 → 賣出
    data.loc[down.rolling(consecutive_days).sum() == consecutive_days, 'Signal'] = -1
    ...
    # === [Step 5] 存檔 TXT ===
    save_analysis_to_txt_consecutive(result, data, consecutive_days)

    return result


def save_analysis_to_txt_consecutive(result, data, consecutive_days):
    filename = f"analysis_results.txt"

    with open(filename, "w", encoding="utf-8") as f:
        f.write(f"--- Consecutive Up/Down ({consecutive_days} days) Analysis ---\n")
