...
def run(stock_symbol, initial_capital):
    ...
    # === [Step 2] 標記 Inside Bar ===
    data['InsideBar'] = (data['High'] < data['High'].shift(1)) & (data['Low'] > data['Low'].shift(1))

    # === [Step 3] 建立交易訊號 ===
    data['Signal'] = 0
    for i in range(1, len(data)):
        if data['InsideBar'].iat[i-1]:  # 前一天是 Inside Bar
            if data['Close'].iat[i] > data['High'].iat[i-1]:
                data.at[data.index[i], 'Signal'] = 1  # 突破 → 買入
            elif data['Close'].iat[i] < data['Low'].iat[i-1]:
                data.at[data.index[i], 'Signal'] = -1  # 跌破 → 賣出

    print("\n=== [Step 3] 加入 InsideBar & Signal 後的資料 ===")
    print(data.head(15))
    ...
    # === [Step 6] 存檔 TXT ===
    save_analysis_to_txt_insidebar(result, data)

    return result


def save_analysis_to_txt_insidebar(result, data):
    filename = "analysis_results_insidebar.txt"

    with open(filename, "w", encoding="utf-8") as f:
        f.write("--- Inside Bar Analysis ---\n")
        f.write("Date\tClose\tHigh\tLow\tOpen\tInsideBar\tSignal\n")
        for i in range(len(data)):
            f.write(f"{data.index[i].date()}\t"
                    ...
                    f"{int(data.iat[i, data.columns.get_loc('InsideBar')])}\t"
